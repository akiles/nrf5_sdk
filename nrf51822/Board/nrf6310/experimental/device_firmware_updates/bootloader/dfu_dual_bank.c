...
void dfu_sd_prepare(uint32_t image_size)
{
    dfu_update_status_t update_status;
    uint32_t            err_code;
    
    // Doing a SoftDevice update thus current application must be cleared to ensure enough space for new SoftDevice.
...
uint32_t dfu_start_pkt_handle(dfu_update_packet_t * p_packet)
{
    ...
    if (m_image_size > DFU_IMAGE_MAX_SIZE_BANKED && !IS_UPDATING_SD())//lint !e655 suppress Lint Warning 655: Bit-wise operations 
    {
        return NRF_ERROR_DATA_SIZE;
    }
    else if (IS_UPDATING_BL() && m_start_packet.bl_image_size > DFU_BL_IMAGE_MAX_SIZE)//lint !e655 suppress Lint Warning 655: Bit-wise operations 
    {
        return NRF_ERROR_DATA_SIZE;
    }
...
static uint32_t dfu_activate_app(void)
{
    ...
    err_code = pstorage_raw_store(&m_storage_handle_app, (uint8_t*) m_storage_handle_swap.block_id, m_start_packet.app_image_size, 0);
...
uint32_t dfu_image_activate()
{
    ...
    {    
        case DFU_STATE_WAIT_4_ACTIVATE:
            ...
            if (IS_UPDATING_SD()) //lint !e655 suppress Lint Warning 655: Bit-wise operations 
            {
                err_code = dfu_activate_sd();
            }
            else if (IS_UPDATING_APP()) //lint !e655 suppress Lint Warning 655: Bit-wise operations 
            {
                err_code = dfu_activate_app();
            }
            else if (IS_UPDATING_BL()) //lint !e655 suppress Lint Warning 655: Bit-wise operations 
            {
                err_code = dfu_activate_bl();
            }
            break;
...
uint32_t dfu_sd_image_swap(void)
{
    ...
    if (bootloader_settings.sd_image_size != 0)
    {
        ...
        sd_mbr_cmd.params.copy_sd.src = (uint32_t *) DFU_BANK_0_REGION_START;
        sd_mbr_cmd.params.copy_sd.dst = (uint32_t *) SOFTDEVICE_REGION_START;
        sd_mbr_cmd.params.copy_sd.len = bootloader_settings.sd_image_size / sizeof(uint32_t);
...
uint32_t dfu_bl_image_swap(void)
{
    ...
    if (bootloader_settings.bl_image_size != 0)
    {
        uint32_t bl_image_start = (bootloader_settings.sd_image_size == 0) ? 
                                  ...
                                  DFU_BANK_0_REGION_START + bootloader_settings.sd_image_size;
...
uint32_t dfu_sd_image_validate(void)
{
    ...
    if (bootloader_settings.sd_image_size != 0)
    {
        ...
        sd_mbr_cmd.params.compare.ptr2 = (uint32_t *) DFU_BANK_0_REGION_START;
